...
public class NetworkModule {
	...
	public NetworkModule(int port) {
		try {
			...
			java.awt.Frame frame = new java.awt.Frame(
					InetAddress.getLocalHost().toString() + ":" + receiverSocket.getLocalPort());
			frame.add(textArea);
	...
	public String listenAsServer() {
		...
		try {
			...
			while (true) {
				...
				// System.out.println( "listening as server for " + playersLeft
				// + " more player" + (playersLeft > 1? "s" : "") );
	...
	public synchronized void addSender(String addressString, InetAddress address, int port) {
		...
		vehicleStates = new State[senderPackets.length + 1]; // for us AND
																// others
